import { starterState } from "./starter-gacha";
...
export async function handleRoster(request: Request, env: Env): Promise<Response> {
  ...
  return json({
    ...
    coins: purse.coins,
    freePulls: 0,
    ...await starterState(env, account.id),
    pullCost: PULL_COST,
...
export async function handleRoll(request: Request, env: Env): Promise<Response> {
  ...
  const wanted = body.pulls;
  if (!Number.isInteger(wanted) || typeof wanted !== "number" || wanted < 1 || wanted > MAX_PULLS_PER_ROLL)
    return fail("bad_count");
  const requestId = body.requestId;
  if (typeof requestId !== "string" || !/^[a-zA-Z0-9-]{16,80}$/.test(requestId)) return fail("request_id_required");
  const prior = await env.DB.prepare("SELECT pulls_json, spent FROM gacha_receipts WHERE account_id = ? AND request_id = ?")
    .bind(account.id, requestId).first<{ pulls_json: string; spent: number }>();
  if (prior) {
    const roster = await handleRoster(request, env);
    return json({ ...await roster.json() as object, pulls: JSON.parse(prior.pulls_json), spent: prior.spent });
  }
  if ((await starterState(env, account.id)).starterEligible) return fail("choose_starter_first", 409);
  ...
  // Validate the snapshot inside the same transaction as the charge and collection writes.
  // A competing purchase cannot spend the same balance or reuse a collection slot.
  statements.unshift(env.DB.prepare(`INSERT INTO gacha_receipts(account_id, request_id, pulls_json, spent, accepted)
    SELECT id, ?, ?, ?, CASE WHEN coins >= ? AND rolls_used = ? THEN 1 ELSE 0 END
    FROM accounts WHERE id = ?`).bind(requestId, JSON.stringify(pulls), cost, cost, purse.rolls_used, account.id));
  try {
    await env.DB.batch(statements);
  } catch (error) {
    const retry = await env.DB.prepare("SELECT pulls_json, spent FROM gacha_receipts WHERE account_id = ? AND request_id = ?")
      .bind(account.id, requestId).first<{ pulls_json: string; spent: number }>();
    if (retry) {
      const roster = await handleRoster(request, env);
      return json({ ...await roster.json() as object, pulls: JSON.parse(retry.pulls_json), spent: retry.spent });
    }
    if (String(error).includes("constraint")) return fail("account_changed_retry", 409);
    throw error;
  }
 ...
 */
export function draw(count: number): PoolEntry[] {
