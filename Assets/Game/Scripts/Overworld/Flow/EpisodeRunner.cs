...
namespace PokeLab.Overworld
{
    ...
    public sealed class EpisodeRunner : MonoBehaviour
    {
        ...
        private IEnumerator Run(Episode episode)
        {
            ...
            // finally hangs off this one bool, because the finally cannot otherwise tell a
            // scene that finished from one that was cut off — an exception in a beat and a
            // StopCoroutine both arrive there by the same door.
            var completed = false;
            ...
            finally
            {
                ...
                // The completion flag is only earned by finishing. It used to be set here
                // unconditionally — the reasoning was that a flag the game depends on should
                // be guaranteed even by a broken scene — but a flag set on an interrupted
                // episode is a save that believes it has seen a scene it has not: the flag
                // gates the beat from ever being offered again, so an exception halfway
                // through the dex handover burned the whole prologue for that save with
                // nothing on screen to say so. StoryEncounter._spoke exists for exactly this
                // reason on its side of the fence — a beat is only retired when it actually
                // played — and this mirrors it: left unset, the StoryTrigger (or the chain
                // resume at startup) can offer the scene again, which is recoverable, where a
                // wrongly-set flag is not.
                if (completed && !string.IsNullOrEmpty(episode.CompletionFlag))
                ...
                var chained = false;
                if (completed && !string.IsNullOrEmpty(episode.NextEpisodeId))
                {
                    chained = Play(episode.NextEpisodeId);
                    if (!chained && !AlreadyPlayed(episode.NextEpisodeId))
                    {
                ...
                // The staged creature is handed to a chained successor rather than destroyed
                // under it: field_bag_left stages the ambusher precisely so field_ambush can
                // walk it over and fight it, and clearing unconditionally here destroyed it in
                // the frame between the two — the approach and the battle then both skipped
                // with warnings and the scene the act is built around silently degraded to
                // text. Every path still ends with a clear, because the last episode of any
                // chain has no successor to chain to.
                if (!chained) ClearStagedCreature();
            }
        }
